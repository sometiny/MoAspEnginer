...
function replacementter(src, starts, ends, callback, state){
	var result="", index = src.indexOf(starts), len = starts.length, lenends = ends.length;
	while(index>=0){
		result += src.substr(0, index);
		var nextindex = src.indexOf(ends, index+len);
		if(nextindex>=0){
			while(src.substr(nextindex-1,1)=="\\"){
				nextindex = src.indexOf(ends,nextindex + lenends);
				if(nextindex<0) break;
			}
			if(nextindex<=0) break;
			var parm = src.substr(index+len,nextindex+lenends-1-index-len);
			result += callback.call(state, parm , starts, ends);
			src = src.substr(nextindex+lenends);
			index = src.indexOf(starts);
		}else{
			break;
		}
	}
...
MoAspEnginerView.perCombine = function(content){
	var _view = new MoAspEnginerView();
	_view.Content = content;
	_view.parsePreCombine();
	_view.parseVari("$");
