...
HomeController = IController.create(function(){
	...
	/*判断是不是关闭了网站*/
	if(Mo.values("System","IsClosed")==1){
		/*网站关闭，显示原因并退出程序，这里的return false是必须的*/
		F.echo(Mo.values("System","Reason"));
		return false;
	}
});
HomeController.extend("Index",function(){
	...
	if((Id=F.get.int("Id",0))>0){
		...
		/*读取当前板块下的留言列表*/
		Model__("Thread","Id").where("PostFor=0 and ForumTag=" + Id).limit(F.get.int("page",1),Mo.values("System","PageSize")).orderby("Id DESC").query().assign("Thread");
		this.display("Forum");
	}else{
		/* 读取所有版块和前五条留言信息*/
	...
	if(Id>0){
		...
		if(!L){
			...
			if(Forum.eof()){
				...
				/*读取所有的回复信息*/
				Model__("Thread","Id").where("PostFor=" +  Id).limit(F.get.int("page",1),Mo.values("System","PageSize")).orderby("Id DESC").query().assign("Reply");
				this.display("Thread");
			}
		}
	}else{
...
HomeController.extend("postmessage",function(){
	/*引用表单验证库*/
	F.require("validatee");
	var fv = new F.exports.validatee();
...
HomeController.extend("code",function(){
	...
	/*直接调用safecode方法*/
	F.exports.safecode("wx_session_code1");
});
</script>