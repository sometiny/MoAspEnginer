...
BDAPI.prototype.GPSConvert = function(coords,from,to){
	...
	if(this.sk!="")this.sn = md5(encodeURIComponent(this.uri+this.sk));
	var http = httprequest.create("http://api.map.baidu.com" + this.uri + "&sn=" + this.sn);
...
BDAPI.prototype.getGPSFromAddress = function(address,city){
	...
	var http = http.create("http://api.map.baidu.com/geocoder/v2/?ak=" + this.ak + "&sn=" + this.sn + "&output=" + this.output + "&address=" + F.encode(address) + "&city=" + F.encode(city));
...
BDAPI.prototype.getAddressFromGPS = function(location,coordtype,pois){
	...
	this.sn="";
	if(this.sk!="")this.sn = md5(this.uri+this.sk);
	var http = httprequest.create("http://api.map.baidu.com/geocoder/v2/?ak=" + this.ak + "&sn=" + this.sn + "&output=" + this.output + "&location=" + F.encode(location) + "&coordtype=" + F.encode(coordtype) + "&pois=" + F.encode(pois));
	return http.getjson("utf-8");
};
