...
WXMessage.News.push(title, content, url, picurl)：添加一条新闻。
*/
DelegateController = IController.create(function(){this.logname = __DIR__ + "\\" + F.date.format(new Date(),"yyyy-MM-dd")+".log";});
...
DelegateController.extend("OnError",function(WX, exception){
	//return; //如果要日志，请注释掉本行
	WriteLog(__DIR__ + "\\exception.log", F.server("QUERY_STRING") + "\r\n" + exception + "\r\n");
});
...
DelegateController.extend("OnReceiveLocation",function(WX, Latitude,Longitude,Precision){
	...
	var address="未知";
	if(F.exports.TXAPI){
		var TXAPI = new F.exports.TXAPI(Mo.C("WX.txkey")),
			Gps = TXAPI.Translate(Latitude + "," + Longitude),
			Pos = TXAPI.Geoencoder(Gps.x + "," + Gps.y);
...
DelegateController.extend("OnViewMenu",function(WX, Url){
	//这里主动调用客服接口，给粉丝发送信息
	F.require("wx/WXServicesKf");
	if(!F.exports.WXServicesKf) return;
	this.KF = new F.exports.WXServicesKf(Mo.C("WX.appid"),Mo.C("WX.appsecret"));
