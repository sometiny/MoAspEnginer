...
ConsoleController.extend("Index", function(){
	...
	if(mo_debug_mode != "SESSION"){
		F.echo(F.format("当前Debug模式为'{0}'，请修改为'SESSION'才可以使用ExceptionManager控制台。<br />请修改配置项MO_DEBUG_MODE的值为'SESSION'，正式上线后建议使用DIRECT模式。", mo_debug_mode));
		return;
	}
