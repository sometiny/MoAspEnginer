...
//****************************************************
MoAspEnginerView.prototype.getLoops = function() {
	F.string.matches(this.Content,/<loop ([\w\.]+)(.*?)>/ig,function($0,$1,$2){
		if($1=="name"){
			this.loops += readAttrs__($1+$2)["name"].replace(/\./ig,"_") + ";"
		}else{
			this.loops += $1.replace(/\./ig,"_") + ";"
		}
	},this);
}
...
MoAspEnginerView.prototype.parseFor = function() {
	F.string.matches(this.Content, /<for (.+?)>/igm ,function($0,$1,$2){
		...
		if(loopname){
			...
			var vbscript = "";
			if(asc){
				vbscript = "{?MoAsp for(var " + keyname + "=0;" + keyname + "<" + loopname + ".length;" + keyname + "++){\r\nvar " + keyvaluename + "=" + loopname + "[" + keyname + "];\r\nMoAsp?}\r\n";
			}else{
				vbscript = "{?MoAsp for(var " + keyname + "=" + loopname + ".length-1;" + keyname + ">=0;" + keyname + "--){\r\nvar " + keyvaluename + "=" + loopname + "[" + keyname + "];\r\nMoAsp?}\r\n";
			}
...
MoAspEnginerView.prototype.parseForeach2 = function() {
	F.string.matches(this.Content, /\<foreach2 ([\s\S]+?)\>/igm ,function($0,$1,$2){
		var attrs = readAttrs__($1);
		if (attrs["name"]) {
	...
	this.Content = F.replace(this.Content, "</foreach2>", "{?MoAsp });\r\n MoAsp?}")
}
...
MoAspEnginerView.prototype.parseForeach = function() {
	F.string.matches(this.Content, /\<foreach ([\s\S]+?)\>/igm ,function($0,$1,$2){
		...
		if(loopname){
			keyname = keyname || "key";
			keyvaluename = keyvaluename || "value";
			var vbscript = "{?MoAsp for(var " + keyname + " in " + loopname + "){\r\nif(!" + loopname + ".hasOwnProperty(" + keyname + "))continue;\r\nvar " + keyvaluename + "=" + loopname + "[" + keyname + "];\r\nMoAsp?}\r\n";
			this.Content = F.replace(this.Content, $0, vbscript);
		}
	},this);
