...
var GLOBAL = this, Exports, F = {
	...
	post : function(key,value){
		...
		if(value === undefined) return (F.__POST_MAPPER__.hasOwnProperty(key.toUpperCase()) ? F.post__[F.__POST_MAPPER__[key.toUpperCase()]] : "");
		F.post__[key] = value;
		F.__POST_MAPPER__[key.toUpperCase()]=key;
		return;
	},
	...
	all : function(key){
		...
		if(F.__POST_MAPPER__.hasOwnProperty(key.toUpperCase())) return F.post__[F.__POST_MAPPER__[key.toUpperCase()]];
		return "";
	},
...
F.post.init = function(){
	if(!F.postinited){
		...
		F.each(Request.Form,function(q){
			...
			F.__POST_MAPPER__[q.toUpperCase()]=q;
		});
		F.postinited = true;
	}
};
...
F.post.remove = function(key){
	F.post.init();
	if(F.__POST_MAPPER__.hasOwnProperty(key.toUpperCase())){
		delete F.post__[F.__POST_MAPPER__[key.toUpperCase()]];
		delete F.__POST_MAPPER__[key.toUpperCase()];
	}
};
F.get.remove = function(key){delete F.get__[key];};
F.post.clear = function(){
	F.post.init();
	delete F.post__;F.post__ = {};
	F.__POST_MAPPER__={};
};
F.get.clear = function(){delete F.get__;F.get__ = {};};

F.post.exists = function(key){F.post.init();return F.__POST_MAPPER__.hasOwnProperty(key.toUpperCase())};
